...
export default async function BusinessConnectCallbackPage({
  searchParams,
}: {
  ...
  const resolvedRequisitionId = requisitionId
    ? requisitionId
    : reference
      ? (await getRequisitions("business")).find((item) => item.reference === reference)
          ?.id ?? ""
      : "";
  ...
  const data = await appFetch<{ requisition: { status: string } }>(
    `/api/requisitions/${resolvedRequisitionId}?workspace=business`,
  );
  ...
  return (
    <AppShell workspace="business">
      <Card className="mx-auto max-w-xl border-border/60 bg-card/70">
        <CardHeader>
          <CardTitle>Business connection in progress</CardTitle>
        </CardHeader>
